	...
	private function check_admin_login() 
	{
			...
			if($query->num_rows() == 1) 
			{
					...
					if($result->user_name===$this->input->post('username') && $result->password===md5($this->input->post('password')))
					{
							...
							$this->session->set_userdata(SESSION.ADMIN_LOGIN_ID,$result->id); 
                            $this->session->set_userdata(SESSION.ADMIN_LOGIN_USERNAME,$result->user_name);
                            $this->session->set_userdata(SESSION.'admin_type',$result->type);
							return 'success';
					}
					else 
					{
