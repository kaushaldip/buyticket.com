...
class Admin extends CI_Controller {
    ...
    {
     	$query = $this->db->get_where('admin_users', array('id' =>$this->session->userdata(SESSION.ADMIN_LOGIN_ID), 'password' => md5($password)));
        $row = $query->row();
        if($query->num_rows() == 0)
        {
     ...
     public function checkpassword()
     {
	  	...
	  	$query = $this->db->get_where('admin_users', array('id' =>$this->session->userdata(SESSION.ADMIN_LOGIN_ID)));
        $row = $query->row();
