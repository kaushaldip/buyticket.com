...
class Paypal{ 
    ...
    function DoDirectPayment($paymentInfo=array()){ 
        ...
        $nvpstr="&PAYMENTACTION=$paymentType&IPADDRESS=$ip&AMT=$amount&CREDITCARDTYPE=$creditCardType&ACCT=$creditCardNumber&EXPDATE=".$padDateMonth.$expDateYear."&CVV2=$cvv2Number&FIRSTNAME=$firstName&LASTNAME=$lastName&STREET=$address1&STREET2=$address2&CITYNAME=$city&STATEORPROVINCE=$state".
        "&POSTALCODE=$zip&COUNTRYCODE=US&CURRENCYCODE=$currencyCode"; 
 ...
 function DoRecuringPayment($paymentInfo=array()){ 
        ...
        $amount = urlencode($paymentInfo['Order']['theTotal']); 
         ...
         $startdate=urlencode($time22.'T00:00:00Z');
         $paymentType=urlencode('Sale'); 
         $profileDesc=urlencode("Website task Monthly Ulitmate");
        $ip=$_SERVER['REMOTE_ADDR']; 
        
        /* Construct the request string that will be sent to PayPal. 
           The variable $nvpstr contains all the variables and is a 
           name value pair string with & as a delimiter */ 
       // $nvpstr="&AMT=$amount&CREDITCARDTYPE=$creditCardType&ACCT=$creditCardNumber&EXPDATE=".$padDateMonth.$expDateYear."&FIRSTNAME=$firstName&LASTNAME=$lastName&PROFILESTARTDATE=$startdate&BILLINGPERIOD=$bperiod&BILLINGFREQUENCY=$bfrequency&CURRENCYCODE=$currencyCode&CVV2=$cvv2Number&POSTALCODE=$zip&COUNTRY=$country&STREET=$address1&STREET2=$address2&CITYNAME=$city&STATEORPROVINCE=$state"; 
          $nvpstr="&AMT=$amount&CREDITCARDTYPE=$creditCardType&ACCT=$creditCardNumber&EXPDATE=".$padDateMonth.$expDateYear."&CVV2=$cvv2Number&FIRSTNAME=$firstName&LASTNAME=$lastName&STREET=$address1&CITY=$city&STATE=$state"."&ZIP=$zip&COUNTRYCODE=US&CURRENCYCODE=$currencyCode&PROFILESTARTDATE=$startdate&DESC=$profileDesc&BILLINGPERIOD=$bperiod&BILLINGFREQUENCY=$bfrequency";
